...
for name in PAPERS:
    for tag, builder in [('mpgem', lambda p: focus(pdf_to_markdown(p))), ('tabula', build_md)]:
        t0=time.time()
        try:
            md=builder(DIR+'/'+name+'.pdf')
            sr=call_json("gemini-pro", P.format(md=md), Rows, system_prompt=SYSTEM, temperature=0.0, max_tokens=16000)
            rows=[r.model_dump() for r in sr.parsed.rows]
            json.dump(rows, open('/tmp/chemx_benz/llm_%s_%s.json'%(tag,name),'w'), ensure_ascii=False)
            print('%-8s %-22s rows=%-4d %.0fs'%(tag,name,len(rows),time.time()-t0), flush=True)
        except Exception as e:
            print('%-8s %-22s ERROR %s'%(tag,name,str(e)[:70]), flush=True)
