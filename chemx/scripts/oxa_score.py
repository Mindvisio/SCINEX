...
base=pd.read_csv('/tmp/ChemX/LLM/result/from_single_agent/oxazolidinone/pred.csv').fillna('NOT_DETECTED'); base['pdf']=base['pdf'].astype(str)
...
def build(titlecase_bact):
    pr=[]
    for stem in PAPERS:
        rows=json.load(open('/root/scinex/chemx/results/llm_%s.json'%stem))
        smap=json.load(open('/root/scinex/chemx/results/oxa_smiles_%s.json'%stem)) if os.path.exists('/root/scinex/chemx/results/oxa_smiles_%s.json'%stem) else {}
        for r in rows:
