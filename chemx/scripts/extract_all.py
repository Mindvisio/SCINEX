...
for name in PAPERS:
    t0=time.time()
    try:
        md=focus(pdf_to_markdown(DIR+'/'+name+'.pdf'))
        sr=call_json("deepseek-v4-pro", P.format(md=md), Rows, system_prompt=SYSTEM, temperature=0.0, max_tokens=15000)
        rows=[r.model_dump() for r in sr.parsed.rows]
        json.dump(rows, open('/tmp/chemx_benz/llm_%s.json'%name,'w'), ensure_ascii=False)
