...
**Режим публичного демо.** Текущий web-endpoint (`/api/chemx/extract`) отдаёт benchmark-replay по Benzimidazoles — готовое проверенное извлечение из `benz_pred_final.csv`; для остальных доменов возвращает пустой результат с пометкой *not wired yet*. Live-извлечение (OCR + LLM + OCSR) на произвольном PDF реализовано в pipeline/CLI, но в этот публичный endpoint пока не подключено.
